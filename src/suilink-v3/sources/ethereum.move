module suilink::ethereum
{
    ...
    public fun ethereum(admin_cap: &suilink::suilink::AdminCap): Ethereum {
    ...
    public fun link_v3(
        ...
        std::string::append_utf8(&mut message, *std::string::bytes(&eth_address));
        ...
        assert!(
            eth_address == suilink::utils::bytes_to_hex(
                ecrecover_eth_address(signature, *std::string::bytes(&message))
            ),
            0
        );
